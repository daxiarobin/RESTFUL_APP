...
v2.0    2019-01-29      Y.Shen  Added longoptname, manstr
'''
...
class InputOpts(object):
	...
	def __init__(self):
		...
		:param
			...
			longoptname -   option name in long format, eg, "node"
			...
			manstr  -       mandatory string, default is O (optional)
			                it can also be M (mandatory) or M|O 
		
		:Usage Example
			from inputopts import InputOpts
			_inputopt = InputOpts()
			_inputopt.optname = 'n'
			_inputop.longoptname = 'node'
